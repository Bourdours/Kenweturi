...
<div class="max-w-4xl mx-auto py-12 px-4 flex flex-col md:flex-row items-center md:items-start gap-12">
  ...
  <div class="hidden md:flex flex-col flex-1">
    <a href="<?= site_url('/') ?>" class="flex items-center gap-2 mb-5">
    ...
    <div class="flex flex-col gap-3">
      <div class="flex items-center gap-3">
        <div class="w-7 h-7 rounded-lg bg-action/15 flex items-center justify-center shrink-0">
          <i class="fa-solid fa-piggy-bank text-action text-xs"></i>
        </div>
        <span class="text-ink/60 text-sm">Économisez sur vos trajets du quotidien</span>
      </div>
      <div class="flex items-center gap-3">
        ...
        <span class="text-ink/60 text-sm">Réduisez votre impact environnemental</span>
      </div>
      <div class="flex items-center gap-3">
        <div class="w-7 h-7 rounded-lg bg-brand/15 flex items-center justify-center shrink-0">
          <i class="fa-solid fa-shield-halved text-brand text-xs"></i>
        </div>
        <span class="text-ink/60 text-sm">Voyagez avec des membres vérifiés</span>
      </div>
    </div>
  </div>
  ...
  <div class="bg-surface rounded-2xl p-7 border border-action/10 w-full md:w-auto md:shrink-0 min-w-80 max-w-[360px]">
    ...
    <?php if (session()->getFlashdata('success')): ?>
      <div class="bg-success/10 border border-success/30 rounded-xl p-3 mb-4 flex gap-2.5 items-center">
        ...
        <span class="text-success text-xs"><?= esc(session()->getFlashdata('success')) ?></span>
      </div>
    <?php endif ?>
