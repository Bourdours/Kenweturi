...
?>
<?= view('partials/head', [
    'extraCss' => $isRecurring ? [] : ['https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'],
    'extraJs'  => $isRecurring ? [] : [
    ...
    <?php if ($isRecurring): ?>
        ...
        <article class="bg-surface-card rounded-2xl p-6 border border-action/10">
            ...
            <ul class="flex flex-col gap-2">
                <?php foreach ($recurringDates as $date): ?>
                    <li class="flex items-center gap-2 text-sm text-ink/70">
                        <i class="fa-solid fa-calendar-day text-action/50 text-xs shrink-0"></i>
                        <?= esc($date) ?> à <?= esc($startTime) ?>
                    </li>
                <?php endforeach ?>
            </ul>
        </article>
